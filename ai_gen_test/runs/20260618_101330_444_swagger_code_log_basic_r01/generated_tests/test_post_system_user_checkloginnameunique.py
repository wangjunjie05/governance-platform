...
def test_post_system_user_checkloginnameunique_case_1():
    url = f"{BASE_URL}/system/user/checkLoginNameUnique"
...
AUTO_NORMAL_CASES = [{'id': 'normal_1', 'path_params': {}, 'query_params': {}, 'body': {'loginName': '创维11'}, 'headers': {'Accept': 'application/json, text/javascript, */*; q=0.01', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'Cookie': 'JSESSIONID=7e0e203e-a4a6-425c-8526-a5c57bed01dc'}, 'actual_path': '/system/user/checkLoginNameUnique', 'expected_status': 200, 'expected_body': True}]

@pytest.mark.parametrize("case", AUTO_NORMAL_CASES, ids=[c['id'] for c in AUTO_NORMAL_CASES])
def test_auto_normal_post_system_user_checkloginnameunique(case):
    url_path = '/system/user/checkLoginNameUnique'
